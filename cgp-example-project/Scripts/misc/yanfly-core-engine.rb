...
#
# 這是一個由Yanfly為RPG Maker VX Ace製作的核心引擎。
# 這個腳本為RPG Maker VX Ace原有的腳本提供一些額外功能和BUG修復。
# 對遊戲GUI的升級和修復。
#
# -----------------------------------------------------------------------------
...
#
# 要安裝該腳本，打開你的腳本編輯器和複製/粘貼此腳本
# 黏貼位置：『▼外掛腳本』以下，『▼入口』以上，記得保存。
#
#==============================================================================
# ▼ Compatibility
#    兼容性
...
#
# 此腳本以RPG Maker VX Ace的規格，嚴格編寫打造，不兼容的狀況極為不可能
# 對於RPG Maker VX不可兼容，除非做出適當的調整
#
#==============================================================================
...
module YEA
  module CORE
    ...
    # - 螢幕解析度調整 -
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    # RPG Maker VX Ace具有較大的寬度和高度的選擇
    # 你的遊戲，調整大小的寬度和高度將有如下變化：
    #
    #               默認      最大值        默認格數           最大格數
    #    寬度       544       640           17                 20
    #    高度       416       480           13                 15
    ...
    #   * 注: 寬度限制為640~110
    #         高度限制為480~010
    #         這是RPG Maker VX Ace遊戲引擎的限制，此腳本無法更改
    #
    # 運用此項設定，所有默認的窗口都會自動調整
    # 但對於非Yanfly系列的腳本，可能會出現不正確調整
    #
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    RESIZE_WIDTH = 544
    RESIZE_HEIGHT = 416
    ...
    # - 數值顯示設定 -
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    # 此值設定為true則遊戲中遊戲中的角色數值，將會變成3個一組
    # 例如：12345將顯示為123,45
    #
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    GROUP_DIGITS = true
    ...
    # FONT_OUTLINE_COLOUR = Color.new(0, 0, 0, 128) efault:   0,   0,   0, 128
    #
    ###########################################################################
    # - 字體設置 -
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    # 說明：
    # FONT_NAME = ["","","",""]      設定字體，如果找不到該字體，將會使用下一個字體
    # FONT_SIZE = 24                 設定字體大小
    # FONT_BOLD = false              設定字型粗體
    # FONT_ITALIC = false            設定字型斜體
    # FONT_SHADOW = false            設定陰影效果
    # FONT_OUTLINE = true            設定輪廓效果
    # FONT_COLOUR = Color.new(255, 255, 255, 255)    設定字體顏色
    # FONT_OUTLINE_COLOUR = Color.new(0, 0, 0, 128)  設定輪廓顏色
    #
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    FONT_NAME = ["VCR OSD Mono", "VL Gothic", "Verdana", "Arial", "Courier"]
    FONT_SIZE = 16
    FONT_BOLD = false
    FONT_ITALIC = false
    FONT_SHADOW = false
    ...
    FONT_OUTLINE_COLOUR = Color.new(0, 0, 0, 255)
    ...
    # - 強制操作設定 -
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    # 想把戰鬥事件強制行動的角色移出行動序列(不執行一般行動)，就打開下面開關。
    # 如果不需要這種功能就把開關編號設為0。
    #
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ...
    # - 計量表外觀設定 -
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    # 你可以修改計量表(HP、MP、TP)出現在你遊戲的方式，
    # 可調節的有：
    # GAUGE_OUTLINE = true   設定輪廓
    # GAUGE_HEIGHT = 5       設定高度
    #
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    GAUGE_OUTLINE = true
    ...
    # - R、L鍵快速切換選單設定 -
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    # RPG Maker VX支持L、R鍵(鍵盤Q、E鍵)快速滾動選單，但是RPG Maker VX Ace
    # 卻禁用這個功能，然而現在經過此腳本，你可以開啟這個功能!
    #
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ...
    # - 系統文本顏色 -
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    # 系統默認的顏色設定HP橙色、MP藍色、TP綠色....等等，有時會讓人覺得很乏味，
    # 然而改變這裡的值，便可以修該相對應的顏色，顏色與數字的關聯性可以由此路徑
    # 『Graphics\System\...』之中的Window.png來知道!
    #
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    COLOURS = {
      :normal => 0, # 默認:  0
    ...
    # 在這裡可以調整：
    #
    # TRANSPARENCY = 160   戰鬥不能角色所顯示的透明度          默認為  160
    # HP_CRISIS = 0.25     HP低於最大值多少時要顯示提示顏色    默認為 0.25
    # MP_CRISIS = 0.25     MP低於最大值多少時要顯示提示顏色    默認為 0.25
    # ITEM_AMOUNT = "×%s"  金幣顯示的前綴
    #
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    TRANSPARENCY = 160
    HP_CRISIS = 0.25
...
#
# ▼以下為腳本源代碼，強烈建議不要自行更改，否則出現：
#   身體不識、腦袋爆炸、電腦報銷、生活不美滿，一概不負責!
#
#==============================================================================
...
class Window_Base < Window
  ...
  #--------------------------------------------------------------------------
  def normal_color
    text_color(YEA::CORE::COLOURS[:normal]);
  end
  ...
  def crisis_color
    text_color(YEA::CORE::COLOURS[:crisis]);
  end

  ;

  def knockout_color
    text_color(YEA::CORE::COLOURS[:knockout]);
  end

  ;

  def gauge_back_color
    text_color(YEA::CORE::COLOURS[:gauge_back]);
  end

  ;

  def hp_gauge_color1
    text_color(YEA::CORE::COLOURS[:hp_gauge1]);
  end

  ;

  def hp_gauge_color2
    text_color(YEA::CORE::COLOURS[:hp_gauge2]);
  end

  ;

  def mp_gauge_color1
    text_color(YEA::CORE::COLOURS[:mp_gauge1]);
  end

  ;

  def mp_gauge_color2
    text_color(YEA::CORE::COLOURS[:mp_gauge2]);
  end

  ;

  def mp_cost_color
    text_color(YEA::CORE::COLOURS[:mp_cost]);
  end

  ;

  def power_up_color
    text_color(YEA::CORE::COLOURS[:power_up]);
  end

  ;

  def power_down_color
    text_color(YEA::CORE::COLOURS[:power_down]);
  end

  ;

  def tp_gauge_color1
    text_color(YEA::CORE::COLOURS[:tp_gauge1]);
  end

  ;

  def tp_gauge_color2
    text_color(YEA::CORE::COLOURS[:tp_gauge2]);
  end

  ;

  def tp_cost_color
    text_color(YEA::CORE::COLOURS[:tp_cost]);
  end
